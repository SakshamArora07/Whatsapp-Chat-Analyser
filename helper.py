...
def fetch_stats(selected_user, df):
    if selected_user == 'Overall':
        ...
        num_messages = df.shape[0] # if whole dataset is selected then return all messages number - no of rows in the dataset

        # 2.number of words
        words = []
        for message in df['user_message']:
            words.extend(message.split()) # splitting the message into words and putting them in the words list
            # using extend pushes all the words separately at the back of the list words but if we use append then it pushes the separated words as a single list at the back of the wrods list
            # extend - ['hi', 'my', 'day']
            # append - ['hi', ['my', 'day']]
        ...
        extract = URLExtract() # for finding urls in a text
        links = []
        for message in df['user_message']:
...
def fetch_most_busy_users(df):
    x = df['user'].value_counts().head() # top 5 unique values/users with most number of rows/messages

    # finding the percentage of messages by each user and round it off till 2 decimal places
    df = round((df['user'].value_counts()/df.shape[0]) * 100, 2).reset_index().rename(columns = {'index': 'name', 'user': 'percent'})
    # reset index - value_counts returns a series with users as the index therefore we convert it into a dataframe with index (users) as a column
...
def create_wordcloud(selected_user, df):
    ...
    with open('stop_hinglish.txt', 'r') as f:
        stop_words = set(f.read().split()) # set ensures all the words are unique

    # using only the required dataframe based on user
    if selected_user == 'Overall':
        temp = df.copy()
    else:
    ...
    def filter_stop_words(message):
        return ' '.join([word for word in message.lower().split() if word not in stop_words]) # words which are in the file/stop_words are removed and words which are not in it are joined again based on space

    temp['filtered_message'] = temp['user_message'].apply(filter_stop_words) # apply the above function on all the entries

    wc = WordCloud(width=500, height=500, min_font_size=10, background_color='white')
    # Generate the word cloud from the filtered messages
    df_wc = wc.generate(temp['filtered_message'].str.cat(sep=' ')) # since the WordCloud method expects a single string therefore we concatenate the string separated by spaces
    return df_wc
...
def most_common_words(selected_user, df):
    if selected_user == 'Overall':
        ...
        words = []
        # including only those words which are not in the stop_words file
...
def timeline_function(selected_user, df):
    ...
    # groups the DataFrame by year, month number, and month name and then counting the number of messages in each group and then using reset_index to convert it back into a dataframe
